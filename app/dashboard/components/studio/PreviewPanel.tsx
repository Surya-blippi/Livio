...
interface PreviewPanelProps {
    ...
    // Credit Estimation (for storyboard preview)
    ...
    const renderContent = () => {
        switch (previewMode) {
            ...
            case 'storyboard':
                ...
                const isFaceMode = mode === 'face' && !!faceImageUrl;

                const estimatedCredits = estimateTotalCredits({
                    mode: isFaceMode ? 'face' : 'faceless',
                    scriptCharCount: scriptLength,
                    sceneCount: sceneCount,
                    includeResearch: false, // Already done if we have scenes
                    includeAssetCollection: false,
                    requiresVoiceCloning: false,
                });

                return (
                    <div className="flex flex-col h-full animate-in slide-in-from-right duration-200">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-subtle)] bg-[var(--surface-1)]">
                            <h3 className="font-bold text-lg text-[var(--text-primary)]">Preview</h3>
                            ...
                            {scenes && scenes.length > 0 ? (
                                <div className="space-y-3">
                                    {scenes.map((scene, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="flex gap-3 p-3 border-2 border-[var(--border-subtle)] rounded-xl bg-[var(--surface-1)] hover:border-gray-400 transition-colors">
                                                {/* Thumbnail */}
                                                <div className="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gray-100 border border-gray-200 relative">
                                                    {thumbnailUrl ? (
                                                        <img src={thumbnailUrl} alt={`Scene ${idx + 1}`} className="w-full h-full object-cover" />
                                                    ) : (
                                                    ...
                                                    {/* Scene number badge */}
                                                    <div className="absolute -top-1 -left-1 w-5 h-5 bg-black text-white rounded-full flex items-center justify-center text-[10px] font-bold">
                                                        {idx + 1}
                                                    </div>
                                                </div>
                                ...
                                <div className="h-full flex flex-col items-center justify-center opacity-50 text-center py-12">
                                    ...
                                    <p className="text-sm">No scenes yet.<br />Generate a script to see the breakdown.</p>
                                </div>
                            )}
                        </div>
                    </div>
